...
const tokenTestAdmin: string = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaab'

export const getMockedUserResponse = (token: string, email: string): typeResponseData|null => {
    if (!token && !email) return null
    let isAdmin: boolean
    if (token === tokenTestUser || email === testUser) isAdmin = false
    else if (token === tokenTestAdmin || email === testAdmin) isAdmin = true
    else return null
