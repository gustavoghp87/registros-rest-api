...
export const getStateOfTerritoryService = async (token: string, territory: string): Promise<stateOfTerritory|null> => {
    ...
    // assigned ?
    const obj: stateOfTerritory|null = await new StateOfTerritoryDb().GetStateOfTerritory(territory)
    return obj
}
...
export const getStateOfTerritoriesService = async (token: string): Promise<stateOfTerritory[]|null> => {
    ...
    if (!user) return null
    const obj: stateOfTerritory[]|null = await new StateOfTerritoryDb().GetStateOfTerritories()
    return obj
}
...
export const changeStateOfTerritoryService = async (token: string, territory: string, isFinished: boolean): Promise<boolean> => {
    ...
    let success: boolean = await new StateOfTerritoryDb().ChangeStateOfTerritory(territory, isFinished)
    if (success) {
...
export const setResetDate = async (territory: string, option: number): Promise<boolean> => {
    ...
    const success: boolean = await new StateOfTerritoryDb().SetResetDate(territory, option)
    return success
}
